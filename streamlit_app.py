...
# Funksjon for å splitte beskrivelsen og flytte data til riktige kolonner
def split_description(data, doc_type):
    if doc_type == "Faktura":
        # Overfør verdier fra Antall_Faktura til Enhet_Faktura hvis enhet finnes
        data['Enhet_Faktura'] = data['Antall_Faktura'].apply(lambda x: x if x in ['M', 'M2', 'STK'] else None)
        # Fjern enhetsverdier fra Antall_Faktura der de ikke er tall
        data['Antall_Faktura'] = data.apply(lambda row: re.search(r'\d+$', row['Beskrivelse_Faktura']).group() if pd.isna(row['Enhet_Faktura']) and re.search(r'\d+$', row['Beskrivelse_Faktura']) else row['Antall_Faktura'], axis=1)
        # Oppdater Beskrivelse_Faktura til å fjerne tallet på slutten
